...
export default function Page() {
  ...
  return <main className="site-shell"><SiteHeader />
    ...
    <section className="section healed-section"><div className="section-label">♡ 03 / {c.healedTitle}</div><div className="healed-head"><h2>{c.healedTitle}</h2><p>{c.healedText}</p></div><div className="healed-grid">{[1, 2, 3].map((i) => <div className="healed-card" key={i}><span>♡</span><small>Replace with healed photo</small></div>)}</div><Link className="text-link" href="/healed">{language === 'fi' ? 'Katso kaikki parantuneet työt' : 'View all healed works'} <ArrowUpRight size={15} /></Link></section>
