...
export default function Page() {
  ...
  return <main className="site-shell"><SiteHeader />
    ...
    <section className="section healed-section"><div className="section-label">♡ 03 / {c.onSkinTitle}</div><div className="healed-head"><h2>{c.onSkinTitle}</h2><p>{c.onSkinText}</p></div><div className="healed-grid">{(onSkinWorks.length ? onSkinWorks.slice(0, 3) : [1, 2, 3].map((i) => ({ title: String(i), image: null }))).map((item: any, index: number) => <div className="healed-card" key={item.id ?? item.title ?? index}>{item.image ? <LightboxImage className="on-skin-preview-image" src={item.image} alt={item.title ?? c.onSkinTitle} /> : <span>♡</span>}<small>{item.image ? item.title : c.onSkinPlaceholder}</small></div>)}</div><a className="text-link" href="/on-skin">{language === 'fi' ? 'Katso kaikki iholla' : 'View all on skin'} <ArrowUpRight size={15} /></a></section>
