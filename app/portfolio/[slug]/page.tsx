...
const PORTFOLIO_QUERY = `
  query PortfolioVisualEdit($relativePath: String!) {
...
function EditableImage({ item, field, label }: { item: any; field: 'sketch' | 'onSkin' | 'healed' | 'image'; label: string }) {
  ...
  return <div className="portfolio-case-media" data-tina-field={tinaField(item, field)}>
    {src ? <LightboxImage className="portfolio-case-image" src={src} alt={`${item.title || 'Portfolio'} ${label}`} /> : <div className="portfolio-case-empty">＋ {label}</div>}
    <span>{label}</span>
  </div>
}
